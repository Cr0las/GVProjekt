...
Mesh generateTextGeometryBuffer(std::string text, float characterHeightOverWidth, float totalTextWidth) {
    ...
    for(unsigned int i = 0; i < text.length(); i++)
    {
        ...
        //Mathias2
        char position = text.at(i);
        mesh.textureCoordinates.at(4 * i + 0) = { float(position) / 128.0, 0.0 };
        mesh.textureCoordinates.at(4 * i + 1) = { (float(position) + 1) / 128.0, 0.0 };
        mesh.textureCoordinates.at(4 * i + 2) = { (float(position) + 1) / 128.0, 1.0 };
        mesh.textureCoordinates.at(4 * i + 3) = { float(position) / 128.0, 1.0 };
